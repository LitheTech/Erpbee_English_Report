...
def get_data(filters):
    ...
    # Main attendance data query grouped by employee
    ...
    for ld in leave_details:
        count = ld['count'] * 0.5 if ld['status'] == "Half Day" else ld['count']
        if ld['employee'] not in leave_map:
            leave_map[ld['employee']] = {}
        leave_map[ld['employee']][ld['leave_type']] = count

    # Construct final data grouped by department
    final_data = []
    ...
    for row in result:
        ...
        # Append leave counts
        for leave_type in leave_types:
            data_row.append(leave_map.get(row['employee'], {}).get(leave_type, 0.0))
